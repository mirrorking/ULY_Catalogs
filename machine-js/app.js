...
const state = {
    ...
    cart: {
        ...
        getScaledImageSize(imgWidth, imgHeight, cellWidthPx, cellHeightPx) {
            ...
            if (imgRatio > cellRatio) {
                width = cellWidthPx;
                height = cellWidthPx / imgRatio;
            } else {
                height = cellHeightPx;
                width = cellHeightPx * imgRatio;
            }
        ...
        async exportToExcel() {
            ...
            exportBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' +
                (state.currentLanguage === 'zh' ? '正在生成Excel...' : 'Generating Excel...');
            ...
            try {
                ...
                worksheet.columns = [
                    ...
                    { header: translations[state.currentLanguage].excel_product_image, key: 'image', width: 25 }, 
                ...
                for (let i = 0; i < this.items.length; i++) {
                    ...
                    for (let col = 1; col <= 10; col++) {
                        ...
                        if (col === 5) {
                            ...
                            cell.alignment = {
                                vertical: 'middle',
                                horizontal: [1, 2, 9, 10].includes(col) ? 'center' : 'left'
                            };
                        }
                ...
                for (let i = 0; i < this.items.length; i++) {
                    ...
                    try {
                        ...
                        if (!imageBuffer) throw new Error('empty image');
                        ...
                        const cellWidthPx = worksheet.getColumn(2).width * 7;
                        const cellHeightPx = row.height * 1.33;

                        const blob = new Blob([imageBuffer], { type: `image/${extension}` });
                        const bitmap = await createImageBitmap(blob);

                        const scale = Math.min(cellWidthPx / bitmap.width, cellHeightPx / bitmap.height);
                        const width = bitmap.width * scale;
                        const height = bitmap.height * scale;

                        worksheet.addImage(imageId, {
                            tl: {
                                col: 1 + (cellWidthPx - width) / cellWidthPx / 2,
                                row: (rowNumber - 1) + (cellHeightPx - height) / cellHeightPx / 2
                            },
                            ext: { width, height },
                            editAs: 'oneCell'
                        });
                ...
                worksheet.getCell(`A${summaryStartRow + 4}`).value =
                    translations[state.currentLanguage].excel_export_date;
                ...
                this.showNotification(
                    state.currentLanguage === 'zh'
                        ? '订购清单已导出为Excel文件'
                        : 'Order list has been exported to Excel'
                );
...
// 轻量翻译函数，返回单条翻译或 undefined
function translate(key) {
    try {
        return translations[state.currentLanguage] && translations[state.currentLanguage][key];
    } catch (e) {
        return undefined;
    }
}
...
async function loadLazyImage(imageContainer, productCode) {
    // 如果已经在加载中或已加载完成，跳过
    if (!imageContainer || !imageContainer.isConnected) return; // 已经从DOM移除，直接跳过
...
function showLazyImage(imageContainer, imageUrl, productCode) {
    ...
    const onLoad = () => {
        try {
            // 只在元素仍连接到文档时修改 DOM
            if (imageContainer && imageContainer.isConnected) {
                imageContainer.classList.remove('loading');
                imageContainer.classList.add('loaded');
                imageContainer.innerHTML = '';
                imageContainer.appendChild(img);
            }
            // 将成功加载的图片地址写回缓存，供详情页和购物车使用
            if (productCode) state.loadedImages[productCode] = imageUrl;
        } catch (e) {
            console.error('showLazyImage onLoad error', e);
        }
    };
    ...
    // 如果图片已在缓存并完成，直接调用 onLoad
    if (img.complete && img.naturalWidth > 0) {
        onLoad.call(img);
    }

    // 超时回退，避免浏览器延迟触发 load 导致一直显示 loading
    const fallback = setTimeout(() => {
        // 只有当元素仍然挂在DOM且未标记为 loaded 时，才显示占位
        if (imageContainer && imageContainer.isConnected && !imageContainer.classList.contains('loaded')) {
            console.warn('showLazyImage timeout for', imageUrl);
            showNoImage(imageContainer, productCode);
        }
    }, 5000);

    const wrap = (fn) => function() { clearTimeout(fallback); fn && fn.apply(this, arguments); };
    img.onload = wrap(onLoad);
    img.onerror = wrap(onError);
}
    ...
    detailContent.innerHTML = `
        <div class="detail-layout">
            <div class="detail-main-wrapper">
                <div class="detail-thumbnails" id="detail-thumbnails" style="width:120px; flex-shrink:0;">
                    <!-- 缩略图列（在主图框内左侧） -->
                </div>
                <div style="flex:1;">
...
function displayPartDetailImage(imageSection, productCode, imageUrl) {
    // 主图容器只包含当前主图
    const mainSection = document.getElementById('detail-image-section');
    ...
    hint.textContent = translate('click_to_zoom') || translate('clickToZoom') || '点击图片放大查看';
    ...
    const onError = () => {
        mainSection.innerHTML = `
            <div class="image-placeholder" style="width: 100%; height: 100%;">
                <i class="fas fa-image"></i>
                <p data-key="image_not_found">图片未找到</p>
                <p style="font-size: 12px; margin-top: 8px;">${productCode}</p>
            </div>
        `;
        translatePage();
    };
    ...
    const fallback = setTimeout(() => {
        if (!img.classList.contains('loaded')) {
            console.warn('displayPartDetailImage timeout', imageUrl);
            onError();
        }
    }, 5000);
    ...
    // 如果缩略图列存在，尝试高亮对应缩略图
    const thumbs = document.querySelectorAll('#detail-thumbnails .thumb-item');
    thumbs.forEach(t => t.classList.remove('active'));
    const related = document.querySelector(`#detail-thumbnails .thumb-item[data-url="${imageUrl}"]`);
...
async function loadMoreImages(productCode) {
    ...
    mainThumb.dataset.url = mainImageUrl;
    mainThumb.title = '主图';

    const mainImgEl = document.createElement('img');
    mainImgEl.alt = '主图';
    mainImgEl.className = 'thumb-img';
    mainImgEl.src = mainImageUrl;

    mainImgEl.onload = () => mainImgEl.classList.add('loaded');
...
async function getMoreImages(productCode) {
    ...
    while (true) {
        ...
        for (const ext of extensions) {
            const imageUrl = `Moreimages/${baseCode}(${imageIndex}).${ext}`;
            const exists = await checkImageExists(imageUrl);
